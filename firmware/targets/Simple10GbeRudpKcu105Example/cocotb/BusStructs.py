...
class reqQp:
    ...
    def mkSimQpAttr(self,
                    pmtu_value,
                    qpState=random.getrandbits(QPA_QPSTATE_B),
                    dqpn_value=random.getrandbits(QPA_DQPN_B),
                    setExpectedPsnAsNextPsn = False,
                    setZero2ExpectedPsnAndNextPsn = False,
                    sqPsn = random.getrandbits(QPA_SQPSN_B),
                    rqPsn = random.getrandbits(QPA_RQPSN_B)
                    ):
        epsn_tmp = rqPsn
        npsn_tmp = sqPsn
        ...
        self.qpaTimeOut           = BitStream(uint = 15, length = QPA_TIMEOUT_B)
